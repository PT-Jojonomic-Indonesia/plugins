...
public class CameraV2 implements Camera {
    ...
    public void open(@NonNull final Result result) throws CameraException {
        try {
            pictureImageReader =
    ...
    private void writeToFile(ByteBuffer buffer, File file) throws IOException {
        ...
        try (FileOutputStream outputStream = new FileOutputStream(file)) {
            ...
            matrix.postRotate((sensorOrientation - getJpegOrientation()) * (isFrontFacing ? -1 : 1));
    ...
    private int getJpegOrientation() {
        int deviceOrientation = currentOrientation;
        if (deviceOrientation == android.view.OrientationEventListener.ORIENTATION_UNKNOWN)
            return 0;
        final CameraCharacteristics c;
        try {
            c = cameraManager.getCameraCharacteristics(cameraName);
